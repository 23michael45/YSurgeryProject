...
namespace MeshTools
{
	internal static class MTEditor_Utility
	{
...
#region Raycast
		 ...
		 * backfaces.
		 */
		public static bool MeshRaycast(Ray InWorldRay, MTMesh mesh, out MTRaycastHit hit)
		{
		 ...
		 */
		public static bool MeshRaycast(Ray InWorldRay, MTMesh mesh, out MTRaycastHit hit, float distance, Culling cullingMode)
		{
			/**
			...
			for(int CurFace = 0; CurFace < mesh.faces.Length; ++CurFace)
			{
				...
				for(int CurTriangle = 0; CurTriangle < Indices.Length; CurTriangle += 3)
				{
					...
					if(!ignore && MTMath.RayIntersectsTriangle(InWorldRay, a, b, c, out dist, out point))
					{
			...
			hit = new MTRaycastHit(OutHitPoint,
		 ...
		 * faces, Culling.Back only tests back faces, and Culling.FrontBack tests both).
		 */
		public static bool MeshRaycast(Ray InWorldRay, MTMesh qe, out List<MTRaycastHit> hits, float distance, Culling cullingMode)
		{
			/**
			 * Transform ray into model space
			 */
			...
			for(int CurFace = 0; CurFace < qe.faces.Length; ++CurFace)
			{
				...
				for(int CurTriangle = 0; CurTriangle < Indices.Length; CurTriangle += 3)
				{
					...
					if(MTMath.RayIntersectsTriangle(InWorldRay, a, b, c, out dist, out point))
					{
						nrm = Vector3.Cross(b-a, c-a);
						...
						switch(cullingMode)
						{
							...
							case Culling.FrontBack:
								hits.Add( new MTRaycastHit(dist,
		 ...
		 */
		public static bool EdgeRaycast(Vector2 mousePosition, ElementCache selection, out MTEdge edge)
		{
			MTMesh mesh = selection.mesh;
			...
			else
			{
				...
				if( MeshRaycast(ray, mesh, out hits, Mathf.Infinity, Culling.FrontBack) )
				{
					...
					for(int i = 0; i < hits.Count; i++)
					{
						...
						foreach(MTEdge e in mesh.faces[hits[i].FaceIndex].GetEdges())
						{
		 ...
		 */
		public static bool PointIsOccluded(MTMesh mesh, Vector3 worldPoint)
		{
			Camera cam = SceneView.lastActiveSceneView.camera;
...
#region Editor
		...
		/**
		 * Attempt to read user data attached to a mesh by MTEditor.
		 */
		 ...
		 */
		public static bool SaveMeshAssetIfNecessary(MTMesh mesh)
		{
			...
			if( !save_path.Equals(DO_NOT_SAVE) && !string.IsNullOrEmpty(save_path) )
			{
				...
				if( existing != null )
				{
					MTMesh_Utility.Copy( (Mesh)existing, mesh.cloneMesh);

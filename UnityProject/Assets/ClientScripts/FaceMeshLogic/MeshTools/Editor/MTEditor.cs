...
namespace MeshTools
{
	 ...
	 */
	public class MTEditor : ScriptableObject, ISerializationCallbackReceiver
	{
#region Const & Members
        ...
        GUIContent gc_SaveSelectVerticesIndices = new GUIContent("Save Vertices Indices", "Save all selected Vertices Indices to Json.");


        public static MTEditor instance { get { return singleton; } }
		[SerializeField] private static MTEditor singleton;

		// A list of pre-formatted indices to be fed to MTMesh_Utility.CreateMesh() to generate
...
#region Initialization 
		...
		[MenuItem("Tools/MeshTools/Edit Selected Mesh &e", true, 0)]
		static bool InitCheck()
		{
			return Selection.transforms.Select(x => x.GetComponentInChildren<MeshFilter>()).FirstOrDefault() != null;
		}
  
		[MenuItem("Tools/MeshTools/Edit Selected Mesh &e", false, 0)]
		static void Init()
		{ 
			if( singleton != null )
				return;

			singleton = ScriptableObject.CreateInstance<MTEditor>();
		...
		void Initialize()
		{
			...
			if( selection.mesh == null )
			{
				...
				if( mf == null )
				{
					MTEditor_Utility.ShowNotification("No Mesh Selected");
		...
		void Finish(ExitStatus status)
		{
			if(status == ExitStatus.DoNotSave || status == ExitStatus.NullValue) 
			{
				if(selection != null && selection.mesh != null)
				{
					...
					{
						if( !EditorUtility.DisplayDialog("Cancel Mesh Modifications", "This will clear all changes to the mesh and exit MeshTools.  Are you sure you wish to continue?", "Yes", "No") )
							return;
			...
			else
			{
				...
				if( !MTEditor_Utility.SaveMeshAssetIfNecessary(selection.mesh) )
				{ 
					return;
				}
		...
		{
			selection.mesh = MTMesh.Create(go);
		...
		{
			return singleton != null ? MTUtil.ValuesWithIndices(singleton.selection.verticesInWorldSpace, singleton.selection.userIndices) : new Vector3[0];
		}
...
#region Scene GUI
		...
		void OnSceneGUI(SceneView scene)
		{
            ...
            Rect rc = GUI.Window(0, windowRect, DrawWindow, "MeshTools");

            windowRect = MTMath.ClampRect(rc , windowBounds );
			...
			if( MTEditor_Utility.SceneViewInUse(currentEvent) )
			{
				return;
			}
                ...
                case EventType.KeyUp:
                    {
                        ...
                        {
                            MTUtil.RecordMeshUndo(selection.mesh, "Delete Faces");
		...
		void SetElementMode(ElementMode mode)
		{
			...
			selection.mesh.handlesRenderer.material = new Material( Shader.Find( mode == ElementMode.Vertex ? "Hidden/MeshTools/VertexShader" : "Hidden/MeshTools/FaceShader") );
		...
		void CacheIndicesForGraphics()
		{
			switch(elementMode)
			{
				...
				{
					List<MTEdge> sel = selection.edges.ToSharedIndex(selection.mesh.triangleLookup).ToList();
		...
		void UpdateGraphics()
		{
			...
			{
				default:
					MTMesh_Utility.MakeFaceSelectionMesh(	ref selection.mesh.handlesRenderer.mesh, 
															MTUtil.ValuesWithIndices(selection.mesh.vertices,
																selectedIndices ));
					break;

				case ElementMode.Vertex:
				{
					MTMesh_Utility.MakeVertexSelectionMesh(
						ref selection.mesh.handlesRenderer.mesh,
						MTUtil.ValuesWithIndices(selection.mesh.vertices, unselectedIndices),
						MTUtil.ValuesWithIndices(selection.mesh.vertices, selectedIndices));
				}
				break;

				case ElementMode.Edge:
				{
					MTMesh_Utility.MakeEdgeSelectionMesh(
						ref selection.mesh.handlesRenderer.mesh,
						MTUtil.ValuesWithIndices(selection.mesh.vertices, unselectedIndices),
						MTUtil.ValuesWithIndices(selection.mesh.vertices, selectedIndices));
				}
		...
		private class HoveringPreview
		{
            ...
            readonly Color FACE_HIGHLIGHT_COLOR = Color.blue;
			readonly Color EDGE_HIGHLIGHT_COLOR = Color.blue;
            readonly Color VERTEX_HIGHLIGHT_COLOR = Color.blue;
            ...
            public void DrawHandles()
			{
				...
				switch(mode)
				{
					case ElementMode.Vertex:
					{
                            ...
                            //Handles.DotCap(-1, vertices[0], Quaternion.identity, HandleUtility.GetHandleSize(vertices[0]) * .6f);
		...
		void UpdateElementHover(Vector2 mousePosition)
		{
			...
			switch( elementMode )
			{
				case ElementMode.Vertex:
				{
					...
					if( MTEditor_Utility.VertexRaycast(mousePosition, CLICK_RECT, selection, out tri) )
					{
						hovering.valid = true;
				...
				case ElementMode.Face:
				{
					...
					if( MTEditor_Utility.MeshRaycast( HandleUtility.GUIPointToWorldRay(mousePosition), selection.mesh, out hit) )
					{
						MTTriangle face = selection.mesh.faces[hit.FaceIndex];
						hovering.valid = true;
				...
				case ElementMode.Edge:
				{
					...
					if( MTEditor_Utility.EdgeRaycast(mousePosition, selection, out edge) )
					{
						hovering.valid = true;
...
#region Vertex Handles
		...
		void VertexHandle()
		{
			...
			if( t_pivot != activePivot )
			{
				if( !movingVertices )
				{
					MTUtil.RecordMeshUndo(selection.mesh, "Move vertices");
...
#region Event Handling
		...
		void OnMouseUp(Event e)
		{
			...
			switch( elementMode )
			{
				case ElementMode.Vertex:
				{
					...
					if( MTEditor_Utility.VertexRaycast(e.mousePosition, CLICK_RECT, selection, out index) )
					{
				...
				case ElementMode.Edge:
				{
					...
					if( MTEditor_Utility.EdgeRaycast(e.mousePosition, selection, out edge) )
					{
						List<MTEdge> sel = selection.edges.Distinct().ToList();
				...
				default:
				{
					...
					if( MTEditor_Utility.MeshRaycast( HandleUtility.GUIPointToWorldRay(e.mousePosition), selection.mesh, out hit) )
					{
						List<MTTriangle> sel = selection.faces;
						MTTriangle tri = selection.mesh.faces[hit.FaceIndex];
		...
		void OnFinishMouseDrag(Vector2 mousePosition)
		{
			...
			switch( elementMode )
			{
				case ElementMode.Face:
				{
					HashSet<MTTriangle> sel = new HashSet<MTTriangle>(selection.faces);

					foreach(MTTriangle tri in selection.mesh.faces)
					{
				...
				case ElementMode.Edge:
				{
					HashSet<MTEdge> sel = new HashSet<MTEdge>(selection.edges);

					foreach(MTEdge edge in selection.mesh.userEdges)
					{
		...
		{
			MTLightmapping.PushGIWorkflowMode();
		...
		{		
			MTLightmapping.PopGIWorkflowMode();
